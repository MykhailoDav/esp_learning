...
bool BMP280::begin(TwoWire &wire) {
    ...
    readCalib();
    write8(0xF4, 0b01010111);   // normal mode, osrs_t x2, osrs_p x16
    write8(0xF5, 0b10110000);   // filter x16, standby 500 ms
    return true;
}
...
float BMP280::readPressure() {
    // readTemperature() must be called first to populate _t_fine
