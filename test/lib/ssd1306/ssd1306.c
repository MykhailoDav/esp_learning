...
void init_ssd1306(void)
{
    ...
    ESP_LOGI(SSD1306_TAG, "Creating I2C master bus...");
    ret = i2c_new_master_bus(&i2c_master_bus_config, &i2c_master_bus);
    if (ret != ESP_OK) {
        ESP_LOGE(SSD1306_TAG, "Failed to create I2C master bus: %s", esp_err_to_name(ret));
        return;
    }
    ESP_LOGI(SSD1306_TAG, "I2C master bus created successfully");
    
    ESP_LOGI(SSD1306_TAG, "Initializing SSD1306 device...");
    ret = i2c_ssd1306_init(i2c_master_bus, i2c_ssd1306_config, &i2c_ssd1306);
    if (ret != ESP_OK) {
        ESP_LOGE(SSD1306_TAG, "Failed to initialize SSD1306: %s", esp_err_to_name(ret));
        return;
    }
    ESP_LOGI(SSD1306_TAG, "SSD1306 initialized successfully");
    
    i2c_ssd1306_buffer_image(&i2c_ssd1306, 32, 0, (const uint8_t *)ssd1306_logo, 64, 32, false);
...
esp_err_t ssd1306_buffer_clear(void)
{
...
esp_err_t i2c_ssd1306_init(i2c_master_bus_handle_t i2c_master_bus, i2c_ssd1306_config_t i2c_ssd1306_config, i2c_ssd1306_handle_t *i2c_ssd1306)
{
    ...
    uint8_t com_pins = 0x02;  // Default for 128x32
    if (i2c_ssd1306_config.height == 64) {
        com_pins = 0x12;  // For 128x64
    }
    ...
    uint8_t ssd1306_init_cmd[] = {
        ...
        OLED_CMD_SET_COM_PIN_HARDWARE_MAP, com_pins,
